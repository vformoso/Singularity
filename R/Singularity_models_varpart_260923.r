...
altitude_record <- cbind(altitude_grid[ , c(3,2)], altitude_range[ , 2])
  colnames(altitude_record)<- c("ID_cells", "mean_altitude", "altitude_range")
  write.csv(altitude_record, "../ALTITUDE_GRID.csv")
...
taxa_climate <- cbind(taxa_df, present_climate[match(taxa_df$sites, present_climate$ID_cell), ])
  taxa_climate <- cbind(taxa_climate, altitude_record$mean_altitude[match(taxa_df$sites, altitude_record$ID_cells)], altitude_record$altitude_range[match(taxa_df$sites, altitude_record$ID_cells)])
...
# 4.2. CLIMATIC AND SPATIAL VARIABLES
...
# current_selected_vars <-  uncor_current_vars$selected.vars 
# current <- rep("Current climate", length(current_selected_vars))
# altitude <- rep("Altitude", length(altitude_vars))
# LGM <- rep("LGM", length(LGM_vars))
# spatial_vars <- c("long", "lat", "I(long * lat)",  "I(long^2)" , "I(lat^2)")
# spatial <- rep("Spatial", length(spatial_vars))
# 
# variables <- c(current_selected_vars, altitude_vars, LGM_vars, spatial_vars)
# by_groups <- c(current, altitude, LGM, spatial)
# 
# groups <- as.data.frame(cbind(variables, by_groups))
# 
# varPart(model = glm_global, groups = groups)          
