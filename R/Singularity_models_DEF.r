...
amph_climate <- cbind(amph_sing, present_climate[match(amph_sing$sites, present_climate$ID_cell), ])
  ...
  amph_climate <- cbind(amph_climate, LGM_climate[match(amph_sing$sites, present_climate$ID_cell), c(2:20)])
...
bird_climate <- cbind(bird_sing, present_climate[match(bird_sing$sites, present_climate$ID_cell), ])
  ...
  bird_climate <- cbind(bird_climate, LGM_climate[match(bird_sing$sites, present_climate$ID_cell), c(2:20)])
...
rept_climate <- cbind(rept_sing, present_climate[match(rept_sing$sites, present_climate$ID_cell), ])
  ...
  rept_climate <- cbind(rept_climate, LGM_climate[match(rept_sing$sites, present_climate$ID_cell), c(2:20)])   
...
mamm_climate <- cbind(mamm_sing, present_climate[match(mamm_sing$sites, present_climate$ID_cell), ])
  ...
  mamm_climate <- cbind(mamm_climate, LGM_climate[match(mamm_sing$sites, present_climate$ID_cell), c(2:20)])
...
# Amphibians
vars <- colnames(amph_climate[ ,c(17:55)]) # Identify climatic variables in the dataframe
  
# Identify weakly correlated variables (R < 0.8) and select the more relevant for the target species, according to VIF
uncor_vars <- corSelect(data = amph_climate, sp.cols = "mean.sing.value", var.cols = vars, cor.thresh = 0.8, select = "VIF")

#Principal variables
vars2 <- c("bio1", "bio5", "bio12", "bio16", "bio17", "altitude", "PMIP_BIO_01", "PMIP_BIO_05", "PMIP_BIO_12", "PMIP_BIO_16", "PMIP_BIO_17")

uncor_vars2 <- corSelect(data = amph_climate, sp.cols = "mean.sing.value", var.cols = vars2, cor.thresh = 0.8, select = "VIF")

# Select predictor variables for GLM using a stepwise procedure
glm_null <- glm(formula = mean.sing.value ~ 1, data = amph_climate, family = "binomial")
glm_maximal <- as.formula(paste("mean.sing.value ~", paste(uncor_vars2$selected.vars, collapse = " + ")))
glm_step <- stepAIC(glm_null, glm_maximal, data = pa_dataEU, direction = "both", trace = FALSE)
